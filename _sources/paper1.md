...
Lorenz and Hartmann 2002 {cite}`lorenz_eddy_2001` make a clear link between how baroclinic storms affect the mean westerly flow through the SAM. We hope here to make similar progress but for ZW3 and ENSO. The link between low-frequency variation, or how ZW3 and ENSO affect the mean geopotential height, and the high-frequecny variance is not clear. This study hopes to shed some light on the direct consequences of phenomenal events.
...
- Corr/cov maps:
    - SAM
        - Fairly linear response, nothing much unexpected. Significant regions found in mean height and variance fields.
    - SOI
        - 
    - ZW3
        - Very few significant regions in either field for ZW3 corr/cov maps, due to small sample size. Using Goyal's method with 1200 years of CESM may solve this.
        - Although no significant regions found, this can be noted and still interpretted with caution. Even it can be suggested that the fact that some ZW3 highs and lows in the mean height field are significant means that the main reason why there are few significant regions is entirely due to the small sample size, and it can be expected that this would change with a greater sample size. Also, as in Goyal et al. 2022, ZW3 explains a greater amount of variance in the meridional wind anomalies field, therefore it can be expected that there would be greater likelihood of significance here -- true also for the MCA analysis.
- Composite Maps:
    - SAM
    - SOI
    - ZW3
...
_Check your understanding: a higher thermal gradient, e.g. in winter, means that the westerly flow is stronger. Does this mean that the faster flow will lose its meridional component faster or slower? i.e. is the polar jet futher equatorward or poleward?_
...
It's interesting that summer has the most clear pattern of covariance, whereas winter, the period when the meridional temperature gradient is steepest and the polar jet is strongest, is much less distinctive. Lower correlation and broken rings.
...
- Goyal finds clear link between ZW3 phase and sea ice extent, which is also connected to the storm track, because a reduction in sea ice will reduce the meridional temeprature gradient, reducing the speed of the polar jet and baroclinic instability, the potential energy store of storm tracks. This is a lagged relationship, as sea-ice extent does not respond immediately, meaning that some of the influence of ZW3 is missed in a simultaneous analysis.
